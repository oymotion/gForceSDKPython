        ...
        if button == 0:
            ...
            while  True:
                ...
                if button == 0:
                    ...
                    GF.setDataNotifSwitch(DataNotifFlags['DNF_QUATERNION'],cmd_cb,1000)
                    ...
                    GF.setDataNotifSwitch(DataNotifFlags['DNF_EMG_RAW'],cmd_cb,1000)
                    GF.startDataNotification(ondata)
                    button = input()
                    if len(button) != 0:
                        GF.stopDataNotification()
                    ...
                    sampRate = eval(input('Please enter sample value(defaults:650): '))
                    channelMask = eval(input('Please enter channelMask value(defaults:0xFF): '))
                    dataLen = eval(input('Please enter dataLen value(defaults:128): '))
                    resolution = eval(input('Please enter resolution value(defaults:8): '))
                    GF.setEmgRawDataConfig(sampRate,channelMask,dataLen,resolution,cb=cmd_cb,timeout=1000)
            break